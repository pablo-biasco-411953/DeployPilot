...
app.MapPost("/api/demo/seed", (IDeployPilotStore store, IConfiguration configuration) =>
{
    ...
    var artifactRoot = configuration["ArtifactSeedRoot"]
        ?? Environment.GetEnvironmentVariable("DEPLOYPILOT_ARTIFACT_SEED_ROOT")
        ?? Path.Combine(AppContext.BaseDirectory, "artifacts");

    AddDemoVersion(store, artifactRoot, inventory.Id, "Inventory Desktop", "1.3.0", "abc123inventory", "Improved build manifests, rollback and integrity validation.", "inventory-desktop-1.3.0.zip");
    AddDemoVersion(store, artifactRoot, billing.Id, "Billing Console", "2.0.4", "abc123billing", "Billing workflow stability update.", "billing-console-2.0.4.zip");
    AddDemoVersion(store, artifactRoot, reports.Id, "Lab Reports", "0.9.8", "abc123reports", "First public beta with version picker support.", "lab-reports-0.9.8.zip");
...
static void AddDemoVersion(IDeployPilotStore store, string artifactRoot, Guid moduleId, string moduleName, string versionNumber, string gitSha, string changelog, string fileName)
{
    var version = store.AddVersion(moduleId, versionNumber, gitSha, changelog);
    var artifact = CreateDemoArtifact(artifactRoot, fileName, moduleName, versionNumber, gitSha);
    store.AddArtifact(version.Id, fileName, $"demo/{fileName}", artifact.Length, artifact.Sha256);
}

static DemoArtifactInfo CreateDemoArtifact(string artifactRoot, string fileName, string moduleName, string versionNumber, string gitSha)
{
    var demoRoot = Path.Combine(artifactRoot, "demo");
    Directory.CreateDirectory(demoRoot);

    var filePath = Path.Combine(demoRoot, fileName);
    ...
    using (var archive = ZipFile.Open(filePath, ZipArchiveMode.Create))
    {
        ...
        writer.WriteLine("DeployPilot demo artifact");
        writer.WriteLine($"Module: {moduleName}");
        writer.WriteLine($"Version: {versionNumber}");
        writer.WriteLine($"Git SHA: {gitSha}");
        writer.WriteLine("This file is generated by the demo seed endpoint.");
    }

    using var stream = File.OpenRead(filePath);
    var hash = SHA256.HashData(stream);
    return new DemoArtifactInfo(new FileInfo(filePath).Length, Convert.ToHexString(hash).ToLowerInvariant());
}

public sealed record DemoArtifactInfo(long Length, string Sha256);
